...
export default function NavigationBar() {
	...
	const navItems = [
		...
		{ name: 'Casos de éxito', href: '#casos-exito', hasDropdown: false },
		{ name: 'Contacto', href: '/contacto', hasDropdown: false },
	];

	const leftNavItems = navItems.slice(0, 2); // Inicio, Servicios
	const rightNavItems = navItems.slice(2); // Casos de éxito, Contacto
		...
		>
			<div className="max-w-7xl mx-auto px-6 lg:px-8">
				<div className="hidden lg:flex items-center justify-evenly h-20">
					...
					<div ref={menuRef} className="flex items-center space-x-8">
						{leftNavItems.map((item) => (
							<div key={item.name} className="relative group">
								<a
									href={item.href}
									className="nav-item block relative px-4 py-2 text-white/90 hover:text-white font-medium transition-all duration-300 group"
									style={{
					...
					{/* Logo - Centered */}
					<div ref={logoRef} className="flex items-center px-8">
					...
					<div className="flex items-center space-x-8">
						{rightNavItems.map((item) => (
							<div key={item.name} className="relative group">
								...
								{/* Dropdown menu (if needed for future items) */}
								{item.hasDropdown && (
									<div className="absolute top-full left-0 mt-2 w-64 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
										<div
											className="rounded-xl p-6 shadow-2xl border border-white/10"
