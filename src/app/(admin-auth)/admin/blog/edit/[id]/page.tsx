  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      if (post) {
        ...
        setLanguage(post.language || "es")
        setPublished(post.published)
        setFeaturedImage(post.featuredImage ?? null)
        setHeadingBackgroundImage(post.headingBackgroundImage ?? null)
        setShowFeaturedImage(post.showFeaturedImage ?? true)
      } else {
  ...
  if (isFetching) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-gobai-cyan"></div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Editar Artículo</h1>
          <p className="text-white/60">Modifica tu publicación existente.</p>
        </div>
      </div>
